...
def parse_dialog_tree(html_file):
    ...
    # Extract synopsis and how to trigger
    metadata = {}
    # Find Synopsis
...
def extract_speaker(element):
    ...
    # If nothing else worked but we have node 92, hardcode it (failsafe)
    if node_id == "92":
        return "Wulbren Bongle"
...
def extract_flags(element, flag_type):
    ...
    # Find the span with the flag class that is a direct child of the current element
    flag_span = element.find('span', class_=flag_type, recursive=False)
    
    if flag_span:
        # Find all flag spans within the main flag span
        inner_spans = flag_span.select(f'.{flag_type}')
        
        for span in inner_spans:
            flag_text = span.text.strip()
            if flag_text:
                # Split by comma and add to flags list
...
def main():
    ...
    html_file = 'data/MOO_Jailbreak_Wulbren.html'
